...
void Init_Radio(void)
{
    ...
    //As  per WDS Calculator Current Frequency Value 30Mhz
    //#define RF_POWER_UP 0x02, 0x81, 0x00, 0x01, 0xC9, 0xC3, 0x80

    //As Suggested Changes to 26Mhz
    //#define RF_POWER_UP 0x02, 0x81, 0x00, 0x01, 0x8C, 0xBA, 0x80
    RadioTransmitBuffer[0] = 0x81;
    RadioTransmitBuffer[1] = 0x00;
    RadioTransmitBuffer[2] = 0x01;
    RadioTransmitBuffer[3] = 0x8C;
    RadioTransmitBuffer[4] = 0xBA;
    RadioTransmitBuffer[5] = 0x80;
...
//		LOG_Info(LOG_YELLOW "CurrentTime=%d",CurrentTime);
	...
	WiSafe_RM_Task_Point_Index = (WiSafe_RM_Task_Point_Index + 1) % sizeof(WiSafe_RM_Task_Point);
					...
					if( (ReceiveRumourAckMessage(id) && AckRumorForUs(id)) || ((ReasonsForRumor&REASON_STATUS_REQUEST)!=0) )//ABR modified
					{
